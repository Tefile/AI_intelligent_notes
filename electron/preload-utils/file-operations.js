...
class FileOperations {
    ...
    async backupToCloud(progressCallback) {
        return cloudSync.backup(progressCallback)
    }

    async restoreFromCloud(progressCallback) {
        return cloudSync.restore(progressCallback)
    }

    async syncToCloud(progressCallback) {
        return cloudSync.sync(progressCallback)
    }

    async testCloudSync() {
        return cloudSync.testConnection()
    }

    async testMysqlSync() {
        return mysqlSync.testConnection()
    }

    async testUnifiedSync(provider, configOverride) {
        const normalized = String(provider || '').trim().toLowerCase()
        if (normalized === 'cloud') return cloudSync.testConnection(configOverride)
        if (normalized === 'mysql') return mysqlSync.testConnection(configOverride)
        throw new Error('Unsupported sync provider')
    }

    async backupSync(provider, progressCallback) {
        const normalized = String(provider || '').trim().toLowerCase()
        if (normalized === 'cloud') return this.backupToCloud(progressCallback)
        if (normalized === 'mysql') return this.backupToMysql(progressCallback)
        throw new Error('Unsupported sync provider')
    }

    async restoreSync(provider, progressCallback) {
        const normalized = String(provider || '').trim().toLowerCase()
        if (normalized === 'cloud') return this.restoreFromCloud(progressCallback)
        if (normalized === 'mysql') return this.restoreFromMysql(progressCallback)
        throw new Error('Unsupported sync provider')
    }

    async runSync(provider, progressCallback) {
        const normalized = String(provider || '').trim().toLowerCase()
        if (normalized === 'cloud') return this.syncToCloud(progressCallback)
        if (normalized === 'mysql') return this.syncToMysql(progressCallback)
        throw new Error('Unsupported sync provider')
    }

    async backupToMysql(progressCallback) {
        return mysqlSync.backup(progressCallback)
    }

    async restoreFromMysql(progressCallback) {
        return mysqlSync.restore(progressCallback)
    }

    async syncToMysql(progressCallback) {
        return mysqlSync.sync(progressCallback)
    }

    async resolveMysqlConflict(options) {
        return mysqlSync.resolveConflict(options)
    }
