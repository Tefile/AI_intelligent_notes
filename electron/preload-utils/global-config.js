...
function buildBuiltinAgentOrchestrationSkill() {
    return {
        ...
        content: [
            ...
            '- `agents_list`：查看当前可用的 Agent。',
            '- `agent_run`：调用指定 Agent 执行单个子任务，并返回过程与结果。',
            '',
            '执行原则：',
            '- 先用 `agents_list` 确认可用 Agent，再选择合适的执行者。',
...
function normalizeSyncConfig(raw) {
    const src = raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {}
    const cloud = src.cloud && typeof src.cloud === 'object' && !Array.isArray(src.cloud) ? src.cloud : {}
    const mysql = src.mysql && typeof src.mysql === 'object' && !Array.isArray(src.mysql) ? src.mysql : {}
    const scope = src.scope && typeof src.scope === 'object' && !Array.isArray(src.scope) ? src.scope : {}
    const provider = typeof src.provider === 'string' ? src.provider.trim().toLowerCase() : 'none'
    const conflictPolicy = typeof src.conflictPolicy === 'string' ? src.conflictPolicy.trim().toLowerCase() : DEFAULT_SYNC_CONFIG.conflictPolicy

    const cloudProvider = typeof cloud.provider === 'string' ? cloud.provider.trim().toLowerCase() : ''
    const allowedCloudProviders = new Set(['generic_s3', 'huawei_obs', 'aliyun_oss', 'volcengine_tos', 'tencent_cos'])

    return {
        enabled: src.enabled === true,
        provider: ['cloud', 'mysql'].includes(provider) ? provider : 'none',
        cloud: {
            provider: allowedCloudProviders.has(cloudProvider) ? cloudProvider : DEFAULT_SYNC_CONFIG.cloud.provider,
            region: typeof cloud.region === 'string' ? cloud.region.trim() : '',
            accessKeyId: typeof cloud.accessKeyId === 'string' ? cloud.accessKeyId.trim() : '',
            secretAccessKey: typeof cloud.secretAccessKey === 'string' ? cloud.secretAccessKey : '',
            bucket: typeof cloud.bucket === 'string' ? cloud.bucket.trim() : '',
            endpoint: typeof cloud.endpoint === 'string' ? cloud.endpoint.trim() : '',
            forcePathStyle: cloud.forcePathStyle === true,
            allowSelfSignedCertificates: cloud.allowSelfSignedCertificates === true,
            objectPrefix: typeof cloud.objectPrefix === 'string' && cloud.objectPrefix.trim()
                ? cloud.objectPrefix.trim()
                : DEFAULT_SYNC_CONFIG.cloud.objectPrefix
        },
        mysql: {
            host: typeof mysql.host === 'string' ? mysql.host.trim() : '',
            port: normalizeOptionalNumber(mysql.port, { min: 1, max: 65535, integer: true }) || DEFAULT_SYNC_CONFIG.mysql.port,
            database: typeof mysql.database === 'string' ? mysql.database.trim() : '',
            username: typeof mysql.username === 'string' ? mysql.username.trim() : '',
            password: typeof mysql.password === 'string' ? mysql.password : ''
        },
        scope: {
            notes: scope.notes !== false,
            noteMeta: scope.noteMeta !== false,
            config: scope.config !== false,
            sessions: scope.sessions === true
        },
        conflictPolicy: ['last_write_wins', 'manual'].includes(conflictPolicy)
            ? conflictPolicy
...
function syncConfigStructure(rawConfig) {
    ...
    const legacyCloudConfig = config.cloudConfig && typeof config.cloudConfig === 'object' && !Array.isArray(config.cloudConfig)
        ? config.cloudConfig
        : {}
    const mergedSyncConfig = normalizeSyncConfig({
        ...(config.syncConfig && typeof config.syncConfig === 'object' && !Array.isArray(config.syncConfig) ? config.syncConfig : {}),
        cloud: {
            ...DEFAULT_SYNC_CONFIG.cloud,
            ...legacyCloudConfig,
            ...(
                config.syncConfig && config.syncConfig.cloud && typeof config.syncConfig.cloud === 'object' && !Array.isArray(config.syncConfig.cloud)
    ...
    return {
        ...
        chatConfig,
        cloudConfig: {
            provider: mergedSyncConfig.cloud.provider,
            region: mergedSyncConfig.cloud.region,
            accessKeyId: mergedSyncConfig.cloud.accessKeyId,
            secretAccessKey: mergedSyncConfig.cloud.secretAccessKey,
            bucket: mergedSyncConfig.cloud.bucket,
            endpoint: mergedSyncConfig.cloud.endpoint,
            forcePathStyle: mergedSyncConfig.cloud.forcePathStyle,
            objectPrefix: mergedSyncConfig.cloud.objectPrefix
        },
        noteConfig: {
            ...noteConfig,
            noteSecurity: {
...
function mergeSyncConfig(current, patch) {
    const base = normalizeSyncConfig(current)
    const src = patch && typeof patch === 'object' && !Array.isArray(patch) ? patch : {}
    return normalizeSyncConfig({
        ...
        mysql: {
            ...base.mysql,
            ...(src.mysql && typeof src.mysql === 'object' && !Array.isArray(src.mysql) ? src.mysql : {})
        },
        scope: {
            ...base.scope,
            ...(src.scope && typeof src.scope === 'object' && !Array.isArray(src.scope) ? src.scope : {})
        }
    })
}
...
function getDefaultUserDataRoot() {
    ...
    // Electron 环境回退：通过 IPC 获取 userData 路径
    try {
        const raw = normalizeExternalPathValue(dbBridge.getUserDataPath())
        if (raw && path.isAbsolute(raw)) return path.resolve(raw)
    } catch { /* fall through */ }
...
function getCurrentUserScopedStorageKey(baseKey) {
    const rootKey = String(baseKey || 'global-config').trim() || 'global-config'
    let userId = 'local-default'
    try {
        const nextId = String(dbBridge.getCurrentUserId?.() || '').trim()
        if (nextId) userId = nextId
    } catch {
        // fall through
    }
    return `${rootKey}:user:${userId}`
}

function getDefaultUserScopedDataRoot() {
    const baseRoot = getDefaultUserDataRoot()
    if (!baseRoot) return ''
    ...
    return path.join(baseRoot, 'users', userId)
}
...
class GlobalConfig {
    constructor() {
        ...
        this._defaultConfig = {
            ...
            timedTask: {},
            dataStorageRoot: getDefaultUserScopedDataRoot(),
            cloudConfig: {
                provider: 'generic_s3',
                region: '',
                accessKeyId: '',
    ...
    _getStorageKey() {
        return getCurrentUserScopedStorageKey(this.STORAGE_KEY)
    }
    ...
    _buildDefaultConfig() {
        return {
            ...this._clone(this._defaultConfig),
            dataStorageRoot: getDefaultUserScopedDataRoot()
        }
    }
    ...
    _getRaw() {
        ...
        let config = dbBridge.getItem(storageKey);
        if (!config) {
            const legacyConfig = dbBridge.getItem(this._getLegacyStorageKey())
            if (this._isPlainObject(legacyConfig)) {
                config = this._clone(legacyConfig)
                dbBridge.setItem(storageKey, config)
            }
        }
        if (!config) {
            return this._buildDefaultConfig();
        }
        ...
        const merged = this._mergeDefaults(normalized, this._buildDefaultConfig());
    ...
    _save(raw) {
        ...
        dbBridge.setItem(this._getStorageKey(), sanitized);
        dbBridge.setItem(`config-meta:${dbBridge.getCurrentUserId() || 'local-default'}:updatedAt`, new Date().toISOString())
    ...
    _ensureWritableDataStorageRoot(raw) {
        ...
        const fallback = getDefaultUserScopedDataRoot()
        const candidates = []
    ...
    exportConfigObject() {
        return this._buildExportableConfig(this._getRaw());
    }
    ...
    importFromObject(parsed) {
        ...
        merged.syncConfig = normalizeSyncConfig(merged.syncConfig)
        const finalMerged = syncConfigStructure(merged)

        const defaultDataRoot = getDefaultUserScopedDataRoot()
        ...
        if (!finalMerged.dataStorageRoot || finalMerged.dataStorageRoot.includes('\0')) {
            finalMerged.dataStorageRoot = defaultDataRoot;
        }
        ...
        finalMerged.syncConfig = normalizeSyncConfig(finalMerged.syncConfig)
    ...
    getSyncConfig() {
        return this._clone(this._getRaw()).syncConfig;
    }

    getConfigUpdatedAt() {
        const userId = String(dbBridge.getCurrentUserId() || 'local-default').trim() || 'local-default'
        return String(dbBridge.getItem(`config-meta:${userId}:updatedAt`) || '').trim()
    }
    ...
    updateConfig(partial) {
        ...
        if (cleanPartial.syncConfig !== undefined && !this._isPlainObject(cleanPartial.syncConfig)) {
            throw new Error('syncConfig must be a plain object');
        }
        ...
        const nextConfig = {
            ...
            syncConfig: cleanPartial.syncConfig !== undefined
                ? mergeSyncConfig(config.syncConfig, cleanPartial.syncConfig)
                : config.syncConfig
        }
    ...
    updateSyncConfig(partial) {
        if (!this._isPlainObject(partial)) {
            throw new Error('partial must be a plain object');
        }
        return this.updateConfig({ syncConfig: partial }).syncConfig;
    }
    ...
    updateCloudConfig(partial) {
        ...
        const nextSyncConfig = mergeSyncConfig(config.syncConfig, {
            cloud: cleanPartial
        })
        const synced = syncConfigStructure({
            ...config,
            syncConfig: nextSyncConfig
        })
        this._save(synced);
        return synced.cloudConfig;
    }
