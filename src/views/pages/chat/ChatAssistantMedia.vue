  ...
  <div :class="['assistant-media', 'chat-page', theme, { 'is-dark': theme === 'dark' }]">
    ...
    <div v-if="msg.imageTask && !msg.imageBubblePlaceholder" class="assistant-image-task">
      <div class="assistant-image-block__header">
        ...
        <div class="assistant-image-task__status-row">
          <span v-if="msg.imageTask.status === 'processing' || msg.imageTask.status === 'in_progress' || msg.imageTask.status === 'running'" class="assistant-image-task__pulse" />
      ...
      <!-- 进度条（处理中显示） -->
      <div v-if="msg.imageTask.status === 'processing' || msg.imageTask.status === 'in_progress' || msg.imageTask.status === 'running'" class="assistant-image-task__progress">
        <div class="assistant-image-task__progress-bar" />
      </div>
...
/* ---------- 任务状态指示器（脉冲动画） ---------- */
.assistant-image-task__status-row {
  display: flex;
  align-items: center;
  gap: 8px;
  flex: 0 0 auto;
...
.assistant-image-task__pulse {
  ...
  animation: assistant-media-pulse 1.6s ease-in-out infinite;
}

@keyframes assistant-media-pulse {
  0% {
...
.assistant-media.is-dark .assistant-image-task__pulse,
:deep(.chat-page.dark) .assistant-image-task__pulse {
  background: rgba(251, 191, 36, 0.95);
  animation-name: assistant-media-pulse-dark;
}

@keyframes assistant-media-pulse-dark {
  0% {
...
/* ---------- 进度条（处理中动画） ---------- */
.assistant-image-task__progress {
  position: relative;
  width: 100%;
  height: 3px;
  border-radius: 999px;
...
.assistant-image-task__progress-bar {
  width: 100%;
  height: 100%;
  border-radius: 999px;
  background: linear-gradient(90deg, rgba(245, 158, 11, 0.3), rgba(251, 191, 36, 0.7), rgba(245, 158, 11, 0.3));
  box-shadow: 0 0 8px rgba(251, 191, 36, 0.4);
  animation: assistant-media-progress 2.8s ease-in-out infinite;
}

@keyframes assistant-media-progress {
  0% {
...
.assistant-media.is-dark .assistant-image-task__progress,
:deep(.chat-page.dark) .assistant-image-task__progress {
  background: rgba(251, 191, 36, 0.16);
}

.assistant-media.is-dark .assistant-image-task__progress-bar,
:deep(.chat-page.dark) .assistant-image-task__progress-bar {
  background: linear-gradient(90deg, rgba(251, 191, 36, 0.4), rgba(253, 224, 71, 0.8), rgba(251, 191, 36, 0.4));
