  ...
  <n-space vertical size="large">
    ...
    <n-card hoverable class="chat-header-card">
      ...
      <n-flex align="center" wrap :size="6" style="margin-top: 10px;">
        ...
        <n-tag
          v-if="activeChatWindow?.activeSessionFilePath || activeSessionFilePath"
          size="small"
          type="primary"
          bordered
          closable
          :title="activeChatWindow?.activeSessionFilePath || activeSessionFilePath"
          @close="closeActiveSession"
        >
          会话：{{ activeChatWindow?.activeSessionTitle || activeSessionTitle || getSessionTitleFromPath(activeChatWindow?.activeSessionFilePath || activeSessionFilePath) }}
        </n-tag>
...
import {
  buildPersistedChatWindowSnapshot,
  readChatWindowsState,
  writeChatWindowsState
} from '@/utils/chatWindowStore'
import {
...
function createEmptyChatWindowState(overrides = {}) {
  const preserveSessionRefs = !!overrides.preserveSessionRefs
  ...
  return {
    ...
    session: {
      messages: preserveSessionRefs ? sessionMessages : deepCopyJson(sessionMessages, []),
      apiMessages: preserveSessionRefs ? sessionApiMessages : deepCopyJson(sessionApiMessages, [])
    },
    activeSessionFilePath: String(overrides.activeSessionFilePath || '').trim(),
    activeSessionTitle: String(overrides.activeSessionTitle || '').trim(),
    selectedAgentId: overrides.selectedAgentId ?? null,
    selectedProviderId: overrides.selectedProviderId ?? null,
    selectedModel: String(overrides.selectedModel || '').trim(),
    basePromptMode: String(overrides.basePromptMode || 'custom'),
    selectedPromptId: overrides.selectedPromptId ?? null,
    customSystemPrompt: String(overrides.customSystemPrompt || ''),
    selectedSkillIds: deepCopyJson(overrides.selectedSkillIds || [], []),
    agentSkillIds: deepCopyJson(overrides.agentSkillIds || [], []),
    activatedAgentSkillIds: deepCopyJson(overrides.activatedAgentSkillIds || [], []),
    manualMcpIds: deepCopyJson(overrides.manualMcpIds || [], []),
    webSearchEnabled: !!overrides.webSearchEnabled,
    autoApproveTools: overrides.autoApproveTools !== undefined ? !!overrides.autoApproveTools : true,
    autoActivateAgentSkills: overrides.autoActivateAgentSkills !== undefined ? !!overrides.autoActivateAgentSkills : true,
    toolMode: String(overrides.toolMode || 'auto'),
    effectiveToolMode: String(overrides.effectiveToolMode || 'expanded'),
    thinkingEffort: String(overrides.thinkingEffort || 'auto'),
    imageGenerationMode: String(overrides.imageGenerationMode || 'auto'),
...
function captureChatWindowSnapshot(options = {}) {
  return createEmptyChatWindowState({
    ...
    activatedAgentSkillIds: activatedAgentSkillIds.value,
    manualMcpIds: manualMcpIds.value,
    ...
    sessionSiderCollapsed: sessionSiderCollapsed.value,
    lastViewedMessageCount: activeChatWindow.value ? activeChatWindow.value.session?.messages?.length || 0 : session.messages.length
  })
}

function snapshotWindowForStorage(windowState, index = 0, activeId = activeChatWindowId.value) {
  const isActive = String(windowState?.id || '') === String(activeId || '')
  const base = isActive ? captureChatWindowSnapshot() : windowState
  const snapshot = buildPersistedChatWindowSnapshot(base, { active: isActive, index })
  return snapshot
}
...
function schedulePersistChatWindowsState() {
  if (windowStateSyncing.value) return
  if (chatWindowsPersistTimer) clearTimeout(chatWindowsPersistTimer)
  chatWindowsPersistTimer = setTimeout(() => {
    chatWindowsPersistTimer = null
    persistChatWindowsState()
  }, 120)
}

function applyChatWindowSnapshot(windowState, options = {}) {
  if (!windowState) return
  windowStateSyncing.value = true
  try {
    const windowSession = windowState.session && typeof windowState.session === 'object' ? windowState.session : {}
    session.messages = Array.isArray(windowSession.messages) ? windowSession.messages : []
    session.apiMessages = Array.isArray(windowSession.apiMessages) ? windowSession.apiMessages : []
    activeSessionFilePath.value = String(windowState.activeSessionFilePath || '').trim()
    activeSessionTitle.value = String(windowState.activeSessionTitle || '').trim()
    selectedAgentId.value = windowState.selectedAgentId ?? null
    selectedProviderId.value = windowState.selectedProviderId ?? null
    selectedModel.value = String(windowState.selectedModel || '').trim()
    basePromptMode.value = String(windowState.basePromptMode || 'custom')
    selectedPromptId.value = windowState.selectedPromptId ?? null
    customSystemPrompt.value = String(windowState.customSystemPrompt || '')
    selectedSkillIds.value = deepCopyJson(windowState.selectedSkillIds || [], [])
    agentSkillIds.value = deepCopyJson(windowState.agentSkillIds || [], [])
    activatedAgentSkillIds.value = deepCopyJson(windowState.activatedAgentSkillIds || [], [])
    manualMcpIds.value = deepCopyJson(windowState.manualMcpIds || [], [])
    webSearchEnabled.value = !!windowState.webSearchEnabled
    autoApproveTools.value = windowState.autoApproveTools !== undefined ? !!windowState.autoApproveTools : true
    autoActivateAgentSkills.value = windowState.autoActivateAgentSkills !== undefined ? !!windowState.autoActivateAgentSkills : true
    toolMode.value = String(windowState.toolMode || 'auto')
    effectiveToolMode.value = String(windowState.effectiveToolMode || 'expanded')
    thinkingEffort.value = String(windowState.thinkingEffort || 'auto')
    imageGenerationMode.value = String(windowState.imageGenerationMode || 'auto')
    videoGenerationMode.value = String(windowState.videoGenerationMode || 'auto')
    input.value = String(windowState.input || '')
    pendingAttachments.value = deepCopyJson(windowState.pendingAttachments || [], [])
    sessionSiderCollapsed.value = windowState.sessionSiderCollapsed !== undefined ? !!windowState.sessionSiderCollapsed : sessionSiderCollapsed.value
    bumpWindowUnreadCounts(windowState.id)
    if (options.resetPickerState) {
      clearInlinePickers()
    }
  } finally {
    windowStateSyncing.value = false
  }
  if (options.resetPickerState) {
...
function ensureChatWindows() {
  ...
  persistChatWindowsState()
  return chatWindows.value
}

function findChatWindow(id) {
  const targetId = String(id || '').trim()
  return chatWindows.value.find((item) => item.id === targetId) || null
}

function saveActiveChatWindowSnapshot() {
  const active = findChatWindow(activeChatWindowId.value)
  ...
  bumpWindowUnreadCounts(active.id)
  persistChatWindowsState()
  return active
}
...
function activateChatWindow(id, options = {}) {
  ...
  target.lastViewedMessageCount = Array.isArray(target.session?.messages) ? target.session.messages.length : 0
  target.unreadCount = 0
  chatWindowRenameDraftId.value = ''
  chatWindowRenameDraft.value = ''
  bumpWindowUnreadCounts(target.id)
  persistChatWindowsState()
}
...
function openSeedChatWindow(seed) {
  ensureChatWindows()
  const text = String(seed?.text || '').trim()
  if (!text) return
  const current = findChatWindow(activeChatWindowId.value)
  if (!current || (current.session.messages.length === 0 && !String(input.value || '').trim())) {
    chatWindowSeed.text = text
    chatWindowSeed.title = String(seed?.title || '').trim()
    chatWindowSeed.source = String(seed?.source || '').trim()
    chatWindowSeed.filePath = String(seed?.filePath || '').trim()
    chatWindowSeed.cellId = String(seed?.cellId || '').trim()
    chatWindowSeed.kind = String(seed?.kind || 'chat-seed').trim()
    input.value = text
  ...
  chatWindowSeed.cellId = String(seed?.cellId || '').trim()
  chatWindowSeed.kind = String(seed?.kind || 'chat-seed').trim()
  activeChatWindowId.value = windowState.id
  input.value = text
  schedulePersistChatWindowsState()
}

function startRenameChatWindow(win) {
  if (!win) return
  chatWindowRenameDraftId.value = win.id
  chatWindowRenameDraft.value = String(win.title || '').trim()
}

function commitRenameChatWindow(win) {
  if (!win) return
  const nextTitle = String(chatWindowRenameDraft.value || '').trim()
  if (nextTitle) {
    win.title = nextTitle
...
function cancelRenameChatWindow() {
  chatWindowRenameDraftId.value = ''
  chatWindowRenameDraft.value = ''
}

function bumpWindowUnreadCounts(activeWindowId = activeChatWindowId.value) {
  const activeId = String(activeWindowId || '').trim()
  chatWindows.value.forEach((item) => {
    const messageCount = Array.isArray(item.session?.messages) ? item.session.messages.length : 0
    if (item.id === activeId) {
      item.lastViewedMessageCount = messageCount
      item.unreadCount = 0
      return
    }
    const lastViewed = Number.isFinite(Number(item.lastViewedMessageCount)) ? Math.max(0, Math.floor(Number(item.lastViewedMessageCount))) : 0
    item.unreadCount = Math.max(0, messageCount - lastViewed)
  })
}
...
function removeDisplayMessageById(messageId, sessionState = session) {
...
onMounted(async () => {
  ensureChatWindows()
  const pendingSeed = consumePendingChatSeed()
  if (pendingSeed?.text) {
    openSeedChatWindow(pendingSeed)
  }
  ...
  window?.addEventListener?.('chat-seed-request', handleChatSeedRequest)
...
function handleChatSeedRequest(event) {
  const seed = event?.detail || null
  if (!seed?.text) return
  openSeedChatWindow(seed)
}
...
function clearSessionData(sessionState = session) {
  const session = sessionState
...
onBeforeUnmount(() => {
  ...
  try {
    window?.removeEventListener?.('chat-seed-request', handleChatSeedRequest)
  } catch {
    // ignore
  }
  try {
...
function clearAllUserEditingState(sessionState = session) {
  const session = sessionState
...
function resolveUserApiIndexForDisplayMessage(msg, sessionState = session) {
  const session = sessionState
...
function getUserApiMessageContentByIndex(apiIndex, sessionState = session) {
  const session = sessionState
...
function findNearestUserApiIndexBefore(apiIndex, sessionState = session) {
  const session = sessionState
...
function findDisplayIndexByApiIndex(role, apiIndex, sessionState = session) {
  const session = sessionState
  return (session.messages || []).findIndex((m) => m?.role === role && m?.apiIndex === apiIndex)
}

function truncateConversationAfterUser(userApiIndex, userDisplayIndex, sessionState = session) {
  const session = sessionState
...
async function runChatRounds({
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    if (
      ...
      assistantDisplay.content = buildEmptyAssistantResponseText(sessionState)
    }
    ...
    for (const toolCall of normalizedToolCalls) {
      throwIfAborted(abortState)
      const exec = await executeToolCall(toolCall, toolMap, lastReasoningText || String(result.reasoning || ''), abortState, sessionState)
      throwIfAborted(abortState)
      session.apiMessages.push({
        role: 'tool',
      ...
      const latestUserPrompt = getLatestRealUserPromptText(session.apiMessages)
...
/**
 * 查询异步图片任务的当前状态
 * 使用 OpenAI 兼容的 GET /images/generations/{task_id} 端点
 */
async function fetchImageGenerationStatus({ requestMeta, imageId, apiKey, signal }) {
  const candidates = buildVideoStatusCandidates(requestMeta?.baseEndpoint, imageId)
  let lastError = null
  ...
  for (const url of candidates) {
    try {
      ...
      if (!resp.ok) {
        ...
        throw new Error(`查询图片任务失败（HTTP ${resp.status}）：${detail || resp.statusText}`)
      }
...
/**
 * 获取异步图片任务完成后的图片内容载荷
 * 优先尝试 GET /images/generations/{id}/content（blob），
 * 若不可用则返回状态轮询的最终 payload
 */
async function fetchImageGenerationContentPayload({ requestMeta, imageId, apiKey, signal }) {
  const candidates = buildVideoContentCandidates(requestMeta?.baseEndpoint, imageId)
  let lastError = null
  ...
  for (const url of candidates) {
    try {
      ...
      if (!resp.ok) {
        const responseText = await resp.text()
        throw new Error(`获取图片内容失败（HTTP ${resp.status}）：${responseText || resp.statusText}`)
      }

      // 尝试获取 JPEG/PNG blob，转为 base64 data URL
      ...
      return {
        data: [
          {
            b64_json: dataUrl.replace(/^data:.*?;base64,/, ''),
...
/**
 * 轮询等待异步图片任务完成
 * 默认每 8 秒轮询一次，最长等待 10 分钟（与视频轮询对称）
 */
async function waitForImageGenerationResult({
  initialPayload,
  requestMeta,
  apiKey,
  signal,
  ...
  const directImages = extractChatImagesFromToolResult(initialPayload)
  if (directImages.length) return initialPayload

  let payload = initialPayload
  let taskState = extractImageGenerationTaskState(payload, requestMeta)
  const imageId = String(taskState?.id || payload?.id || payload?.task_id || '').trim()
  if (!imageId) return payload
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (['completed', 'succeeded', 'success'].includes(status)) {
      if (extractChatImagesFromToolResult(payload).length) return payload
      // 任务完成但无图片数据，尝试获取内容载荷
      const contentPayload = await fetchImageGenerationContentPayload({ requestMeta, imageId, apiKey, signal })
    ...
    const nextPayload = await fetchImageGenerationStatus({ requestMeta, imageId, apiKey, signal })
    if (!nextPayload) return payload
    payload = nextPayload
    taskState = extractImageGenerationTaskState(payload, requestMeta)
    if (!taskState && extractChatImagesFromToolResult(payload).length) return payload
  }

  // 超时：返回当下的 payload（可能仍有图片数据）
  return payload
}
...
async function runImageGenerationRound({
  ...
  const prompt = buildImageGenerationPromptFromHistory(userPrompt, { sessionState })
  ...
  if (imageTask) {
    // 异步图片任务：先显示任务受理状态，然后轮询等待结果
    applyImageGenerationTaskToDisplay(assistantDisplay, imageTask, placeholderMode)
    await scrollToBottom()

    // 轮询等待任务完成（最长 10 分钟，每 8 秒一次）
    const resolvedPayload = await waitForImageGenerationResult({
    ...
    if (!resolvedPayload) {
      // 轮询超时：保留任务受理状态，提示用户稍后查看
      assistantDisplay.imageTask = {
        ...imageTask,
        status: 'timeout',
        note: '图片任务轮询超时（10分钟），请检查服务商控制台确认任务状态。'
      }
      session.apiMessages.push({
        role: 'assistant',
        content:
          placeholderMode === 'image'
            ? `图片任务已受理（轮询超时）：${assistantImageTaskStatusLabel(assistantDisplay)}${imageTask.id ? `（任务 ID：${imageTask.id}）` : ''}`
            : buildImageGenerationPendingText({ ...imageTask, status: 'timeout' })
      })
      assistantDisplay.apiIndex = session.apiMessages.length - 1
      setCurrentAssistantDisplay(null)
      await scrollToBottom()
    ...
    // 检查轮询结果中是否有图片
    const resolvedImages = extractChatImagesFromToolResult(resolvedPayload)
    if (resolvedImages.length) {
      const revisedPrompts = collectImageGenerationRevisedPrompts(resolvedPayload)
      applyImageGenerationImagesToDisplay(assistantDisplay, { images: resolvedImages, userPrompt, revisedPrompts })
      session.apiMessages.push({
        role: 'assistant',
    ...
    // 轮询返回但无图片：尝试从轮询结果提取文本
    const resolvedText = extractImageGenerationTextResult(resolvedPayload)
    if (resolvedText) {
      applyImageGenerationTextToDisplay(assistantDisplay, resolvedText)
      session.apiMessages.push({ role: 'assistant', content: resolvedText })
      assistantDisplay.apiIndex = session.apiMessages.length - 1
      setCurrentAssistantDisplay(null)
      await scrollToBottom()
      return
...
async function runVideoGenerationRound({
  ...
  const prompt = buildVideoGenerationPromptFromHistory(userPrompt, { sessionState })
  const { payload, requestMeta } = await withTimeout(
    postVideoGeneration({
      baseUrl,
      apiKey,
...
// 图片生成请求超时时间：默认 10 分钟（600000ms）
// 异步图片任务（如 DALL-E 等）需要较长的等待时间
const IMAGE_GENERATION_REQUEST_TIMEOUT_MS = 600000
...
async function prepareUserApiMessage({ text, attachments, userDisplay, preferVision = true, providerKind = 'openai-compatible', sessionState = session }) {
  const session = sessionState
...
async function regenerateAssistant(msg) {
  ...
  const sessionState = {
    ...
    apiMessages: Array.isArray(session.apiMessages) ? session.apiMessages : []
  }
  ...
  const assistantApiIndex = isFiniteNumber(msg?.apiIndex)
    ? msg.apiIndex
    : (() => {
        for (let i = (sessionState.apiMessages || []).length - 1; i >= 0; i--) {
          if (sessionState.apiMessages[i]?.role === 'assistant') return i
        }
        return -1
      })()
  ...
  const userDisplayIndex =
    findDisplayIndexByApiIndex('user', userApiIndex) >= 0
      ...
      : (() => {
          const assistantDisplayIndex = (sessionState.messages || []).findIndex((m) => m?.id === msg?.id)
          if (assistantDisplayIndex <= 0) return -1
          for (let i = assistantDisplayIndex - 1; i >= 0; i--) {
            if (sessionState.messages[i]?.role === 'user') return i
          }
          return -1
        })()
  ...
  const hasFollowing = sessionState.messages.length > userDisplayIndex + 1 || sessionState.apiMessages.length > userApiIndex + 1
  ...
  try {
    const userDisplay = sessionState.messages[userDisplayIndex]
    const triggerText = [
...
async function submitUserEdit(msg) {
  ...
  const userApiIndex = resolveUserApiIndexForDisplayMessage(msg, sessionState)
  ...
  const userDisplayIndex = (sessionState.messages || []).findIndex((m) => m?.id === msg?.id)
  if (userDisplayIndex < 0) {
  ...
  if (sessionState.apiMessages?.[userApiIndex]?.role === 'user') {
    sessionState.apiMessages[userApiIndex].content = mergeUserTextWithExistingAttachments(
      sessionState.apiMessages[userApiIndex].content,
      draft
    )
  }
...
function buildRequestApiMessages(providerKind = 'openai-compatible', options = {}) {
  const { tools = [], reservedCharsOverride = null } = options || {}
  const sessionState = options.sessionState || session
  return buildChatContextWindow(
    sessionState.apiMessages,
...
function buildEmptyAssistantResponseText(sessionState = session) {
  const session = sessionState
  const imageMode = normalizeImageGenerationMode(imageGenerationMode.value)
  const videoMode = normalizeImageGenerationMode(videoGenerationMode.value)
  const model = String(selectedModel.value || '').trim()
...
function buildRequestMessages(options = {}) {
  ...
  const sourceMessages = Array.isArray(apiMessages) ? apiMessages : buildRequestApiMessages('openai-compatible', { tools, sessionState })
...
async function executeToolCall(toolCall, toolMap, lastReasoningText, abortState = null, sessionState = session) {
  const session = sessionState
  throwIfAborted(abortState)
...
async function send() {
  ...
  sessionState.messages.push(userDisplay)
  autoScrollEnabled.value = true
  scheduleRefreshUserAnchorMeta()
  await scrollToBottom({ force: true })
  ...
  bumpWindowUnreadCounts(activeChatWindowId.value)
  saveActiveChatWindowSnapshot()
}
