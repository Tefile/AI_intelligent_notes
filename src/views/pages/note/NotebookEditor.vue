  ...
  <div v-else ref="editorRootRef" :class="['notebook-editor', { 'is-dark': theme === 'dark' }]">
    <header class="notebook-editor__header">
      ...
      <div class="notebook-editor__toolbar">
        ...
        <n-tooltip trigger="hover" v-if="props.filePath"><template #trigger><n-button secondary circle size="small" @click="sendToJupyterChatbook"><template #icon><n-icon><ChatMultiple24Filled /></n-icon></template></n-button></template>JupyterChatbook：发送选中内容到聊天</n-tooltip>
...
import { requestChatSeed } from '@/utils/chatSeedBridge'
...
const jupyterChatbookSelectedText = computed(() => {
  const selectedCell = notebook.value.cells.find((cell) => cell.id === selectedCellId.value)
  const selectedComponent = resolveCellComponent(selectedCell?.id)
  if (selectedCell?.cell_type === 'markdown') {
    const selectedText = String(selectedComponent?.getSelectedText?.() || '').trim()
    if (selectedText) return selectedText
  }
  return String(selectedCell?.source || '').trim()
})
...
function sendToJupyterChatbook() {
  const text = String(jupyterChatbookSelectedText.value || '').trim()
  if (!text) {
    message.warning('当前没有可发送到 JupyterChatbook 的内容。')
    return
  }

  requestChatSeed({
    kind: 'jupyter-chatbook',
    text,
