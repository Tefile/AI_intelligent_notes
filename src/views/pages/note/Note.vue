<template>
  <n-space vertical size="large" :class="['note-page', { 'is-dark': theme === 'dark' }]">
        ...
        >
          <template v-if="openNotes.length">
            ...
            <div class="note-page__editor-pane">
              <div v-if="activeNoteSecurityStatus" class="note-page__security-banner">
                <div class="note-page__security-banner-copy">
                  <n-text strong>{{ activeNoteSecurityStatus.title }}</n-text>
                  <n-text depth="3">{{ activeNoteSecurityStatus.description }}</n-text>
                </div>
                <div class="note-page__security-banner-badges">
                  <span>{{ activeNoteSecurityStatus.typeLabel }}</span>
                  <span>{{ activeNoteSecurityStatus.recoveryLabel }}</span>
                  <span>{{ activeNoteSecurityStatus.scopeLabel }}</span>
                </div>
              </div>
              <component
    ...
    >
      <n-flex vertical :size="12" :class="['note-page__modal-panel', { 'is-dark': theme === 'dark' }]">
        <div class="note-page__security-summary">
          <div class="note-page__security-summary-copy">
            <n-text strong>{{ setPasswordSecurityTitle }}</n-text>
            <div class="note-page__modal-desc">目标笔记：[{{ setPasswordModal.path }}]</div>
            <n-text depth="3">{{ setPasswordSecurityDescription }}</n-text>
          </div>
          <div class="note-page__security-badges">
            <span>{{ setPasswordModalModeTitle }}</span>
            <span>{{ setPasswordRecoveryBadge }}</span>
            <span>作用域：当前用户</span>
          </div>
        </div>
        <div class="note-page__security-section">
          <div class="note-page__security-section-header">
            <n-text strong>笔记密码</n-text>
            <n-text depth="3">笔记内容依然由笔记密码直接解锁，全局配置密码只作为可选恢复凭据。</n-text>
          </div>
          <n-input
        ...
        <div v-if="hasGlobalFallbackPassword" class="note-page__security-section note-page__security-section--soft">
          <div class="note-page__security-section-header">
            <n-text strong>全局恢复绑定</n-text>
            <n-text depth="3">{{ setPasswordRecoveryDescription }}</n-text>
          </div>
          <n-alert :type="setPasswordHasExistingRecovery ? 'success' : 'info'" :show-icon="false">
            {{ setPasswordRecoveryAlert }}
          </n-alert>
          <n-input
    ...
    >
      <n-flex vertical :size="12" :class="['note-page__modal-panel', { 'is-dark': theme === 'dark' }]">
        <div class="note-page__security-summary">
          <div class="note-page__security-summary-copy">
            <n-text strong>通过全局配置密码重置</n-text>
            <div class="note-page__modal-desc">目标笔记：[{{ resetPasswordModal.path }}]</div>
            <n-text depth="3">{{ resetPasswordSecurityDescription }}</n-text>
          </div>
          <div class="note-page__security-badges">
            <span>前提：已绑定恢复</span>
            <span>动作：重置笔记密码</span>
        ...
        <div class="note-page__security-section">
          <div class="note-page__security-section-header">
            <n-text strong>验证与新密码</n-text>
            <n-text depth="3">这个操作不会修改全局配置密码，只会为当前笔记设置新的笔记密码，并继续保留全局恢复能力。</n-text>
          </div>
          <n-input
...
const activeNoteSecurityStatus = computed(() => {
  const notePath = normalizeNotePath(activeNote.value?.path)
  if (!notePath) return null
  const meta = getProtectedNoteMeta(notePath)
  if (!meta) {
    return {
      title: '当前笔记未加密',
      description: '当前以明文方式存储。需要时可在右侧文件树中为这本笔记设置密码保护。',
      typeLabel: '状态：明文',
      recoveryLabel: hasGlobalFallbackPassword.value ? '恢复：未绑定全局恢复' : '恢复：未配置全局恢复',
      scopeLabel: '作用域：当前用户'
    }
  }
  return {
    title: meta.hasFallbackRecovery ? '当前笔记已加密并绑定全局恢复' : '当前笔记已加密',
    description: meta.hasFallbackRecovery
      ? '这本笔记由笔记密码直接解锁，同时已绑定全局配置密码，可用于当前用户下的密码重置。'
      : '这本笔记只能通过笔记密码直接解锁。如需全局恢复，请重新设置笔记密码并补绑全局配置密码。',
    typeLabel: '状态：已加密',
    recoveryLabel: meta.hasFallbackRecovery ? '恢复：已绑定全局恢复' : '恢复：仅笔记密码',
    scopeLabel: '作用域：当前用户'
  }
})
...
const setPasswordHasExistingRecovery = computed(() => {
  const meta = getProtectedNoteMeta(setPasswordModal.value.path)
  return meta?.hasFallbackRecovery === true
})
const setPasswordSecurityTitle = computed(() => {
  return setPasswordModalModeTitle.value === '修改笔记密码'
    ? '更新当前笔记保护'
    : '为当前笔记启用保护'
})
const setPasswordSecurityDescription = computed(() => {
  return setPasswordModalModeTitle.value === '修改笔记密码'
    ? '你可以更新笔记密码，并按需保留或补绑全局恢复能力。'
    : '你可以先设置笔记密码，再决定是否额外绑定全局恢复能力。'
})
const setPasswordRecoveryBadge = computed(() => {
  if (!hasGlobalFallbackPassword.value) return '未配置全局恢复'
  return setPasswordHasExistingRecovery.value ? '已绑定全局恢复' : '可选绑定全局恢复'
})
const setPasswordRecoveryDescription = computed(() => {
  if (setPasswordHasExistingRecovery.value) {
    return '当前笔记已经绑定全局恢复。留空会继续保留现有绑定，重新输入一次全局配置密码则会按当前全局凭据重新封装。'
  }
  return '当前笔记还没有绑定全局恢复。输入一次全局配置密码后，后续才可以通过全局配置密码重置此笔记密码。'
})
const setPasswordRecoveryAlert = computed(() => {
  if (setPasswordHasExistingRecovery.value) {
    return '这本笔记已经可以走“全局配置密码重置笔记密码”流程。'
  }
  return '这本笔记目前只能靠笔记密码解锁；如果要支持全局恢复，请在下方额外输入一次全局配置密码。'
})
const resetPasswordSecurityDescription = computed(() => {
  return '只适用于已经绑定过全局恢复的加密笔记。验证通过后，会把当前用户下这本笔记的密码重设为新密码。'
})
...
function handleResetPasswordRequest(filePath) {
  ...
  if (protectedMeta.hasFallbackRecovery !== true) {
    message.warning('该笔记尚未绑定全局恢复，不能直接使用全局配置密码重置')
    return
  }
  resetPasswordModal.value = {
    show: true,
...
.note-page__security-banner {
  ...
  background:
    ...
    radial-gradient(circle at top right, rgba(14, 165, 233, 0.08), transparent 54%);
}

.note-page__security-banner-copy {
  display: flex;
  flex: 1;
  min-width: 220px;
  flex-direction: column;
...
.note-page__security-banner-badges {
  display: flex;
  flex-wrap: wrap;
...
.note-page__security-banner-badges span {
  padding: 5px 10px;
  border-radius: 999px;
...
.note-page.is-dark .note-page__security-banner {
  border-color: rgba(148, 163, 184, 0.16);
  background:
    linear-gradient(145deg, rgba(15, 23, 42, 0.92), rgba(30, 41, 59, 0.88)),
    radial-gradient(circle at top right, rgba(56, 189, 248, 0.16), transparent 54%);
}

.note-page.is-dark .note-page__security-banner-badges span {
...
.note-page__security-summary-copy {
  display: flex;
  flex-direction: column;
  gap: 6px;
  min-width: 0;
...
.note-page__security-badges {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}

.note-page__security-badges span {
  padding: 5px 10px;
  border-radius: 999px;
  border: 1px solid rgba(148, 163, 184, 0.18);
  background: rgba(255, 255, 255, 0.78);
...
.note-page__security-section--soft {
  background: rgba(241, 245, 249, 0.78);
}

.note-page__security-section-header {
  display: flex;
  flex-direction: column;
  gap: 4px;
}
...
.note-page.is-dark .note-page__security-summary,
.note-page__modal-panel.is-dark .note-page__security-summary {
  border-color: rgba(148, 163, 184, 0.16);
  background:
    linear-gradient(145deg, rgba(15, 23, 42, 0.92), rgba(30, 41, 59, 0.88)),
    radial-gradient(circle at top right, rgba(56, 189, 248, 0.16), transparent 54%);
...
.note-page.is-dark .note-page__security-badges span,
.note-page__modal-panel.is-dark .note-page__security-badges span {
  border-color: rgba(148, 163, 184, 0.18);
  background: rgba(15, 23, 42, 0.54);
  color: rgba(226, 232, 240, 0.92);
}

.note-page.is-dark .note-page__security-section,
.note-page__modal-panel.is-dark .note-page__security-section {
  border-color: rgba(148, 163, 184, 0.14);
  background: rgba(15, 23, 42, 0.56);
}

.note-page.is-dark .note-page__security-section--soft,
.note-page__modal-panel.is-dark .note-page__security-section--soft {
