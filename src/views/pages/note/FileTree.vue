<template>
  ...
  <div :class="['note-tree', { 'is-dark': theme === 'dark' }]" @contextmenu.prevent="handleTreeContextMenu">
    ...
    >
      ...
      :render-prefix="renderPrefix"
      :render-label="renderPickerLabel"
        />
...
function renderLabel({ option }) {
  ...
  const status = getNodeSecurityStatus(option);
  const children = [h('span', { class: 'tree-node-label__text', title: labelText }, labelText)];
  if (status) {
    children.push(
      h('span', {
        class: ['tree-node-security-badge', status.hasGlobalRecovery ? 'is-recoverable' : 'is-protected'],
        title: status.hasGlobalRecovery ? '已加密，且已绑定全局恢复' : '已加密，仅可使用笔记密码解锁'
      }, status.hasGlobalRecovery ? '可恢复' : '已加密')
    );
  }
  return h('span', { class: 'tree-node-label' }, children);
}

function renderPickerLabel({ option }) {
  const labelText = typeof option.label === 'string' ? option.label : String(option.label ?? '');
  return h('span', { class: 'tree-node-label tree-node-label--plain' }, [
    h('span', { class: 'tree-node-label__text', title: labelText }, labelText)
  ]);
}

function getProtectedNoteMeta(filePath) {
  const normalized = String(filePath || '').trim().replace(/\\/g, '/');
  if (!normalized || !normalized.startsWith('note/')) return null;
  const protectedNotes = props.noteSecurity?.protectedNotes && typeof props.noteSecurity.protectedNotes === 'object'
    ? props.noteSecurity.protectedNotes
    : {};
  return protectedNotes[normalized] || null;
}

function getNodeSecurityStatus(node) {
  if (!node?.isLeaf) return null;
  const meta = getProtectedNoteMeta(node.key);
  if (!meta) return null;
  return {
    protected: true,
    hasGlobalRecovery: meta.hasFallbackRecovery === true
  };
}
...
.tree-node-label--plain {
  display: inline-flex;
  width: auto;
}

.tree-node-label__text {
  flex: 1 1 auto;
  min-width: 0;
  overflow: hidden;
...
.tree-node-security-badge.is-recoverable {
  border-color: rgba(16, 185, 129, 0.2);
  background: rgba(220, 252, 231, 0.92);
  color: rgba(4, 120, 87, 0.96);
}

.note-tree.is-dark .tree-node-security-badge {
  border-color: rgba(56, 189, 248, 0.24);
  background: rgba(8, 47, 73, 0.78);
  color: rgba(125, 211, 252, 0.96);
}

.note-tree.is-dark .tree-node-security-badge.is-recoverable {
